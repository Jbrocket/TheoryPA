...
class NFA:
    ...
    def __init__(self, states, alphabet, start_state, accept_states, transitions):
        ...
        for transition in self.transitions:
            if transition.state_from not in states:
                raise ValueError(f'state from in {transition} is not in set of states')
            if transition.symbol not in alphabet and transition.symbol != EPSILON:
                raise ValueError(f'symbol in {transition} is not in alphabet')
            if transition.state_to not in states:
                raise ValueError(f'state to in {transition} is not in set of states')
...
def _read_transition(line):
    state_from, symbol, state_to = line.split()
    return Transition(state_from, symbol, state_to)

def write_nfa(M, fout):
    ...
    for transition in M.transitions:
        print(f'{transition.state_from} {transition.symbol} {transition.state_to}', file=fout)
